...
class VSC(DAEModel):
    def __init__(self, *args, **kwargs):
        super().__init__(*args, **kwargs) #Refers to the parent class DAEModel arguments and keyword arguments
        #Creating a function with two arrays with zeros and of size self.n_units and data type structure define by bus_ref_spec
    ...
    #Method to return the bus value where vsc is connected
    def bus_ref_spec(self):
        return {'terminal': self.par['bus']}
    #Function to generate reference currents I_d and I_q. The actual PI and lag filters are defined elsewhere, only error is computed here
    def add_blocks(self):
        p = self.par
        ...
        #Controller includes a lag filter for low frequency gain enhancement or for inner current loop dynamics?
    ...
    def P(self, x, v):
        v_n = self.sys_par['bus_v_n'][self.bus_idx_red['terminal']]   #Obtaining base value?
